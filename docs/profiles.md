...
Profiles can also declare `git_hooks` entries (global-scope, always run; or trusted-scope, gated on the repo's remote matching `git_hooks_trusted_remotes`), materialized into a single global `core.hooksPath` dispatcher — off by default unless at least one profile declares an entry. See the "Git Hooks" section of `roles/gitconfig/README.md` for the schema and the trust model.
