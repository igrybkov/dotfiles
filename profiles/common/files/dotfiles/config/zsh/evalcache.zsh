...
function _evalcache_autocomplete () {
  local cmdHash="nohash" data="$*" name
  ...
  local cacheFile="$ZSH_EVALCACHE_DIR/completion/cache/${name##*/}_${cmdHash}.zsh"
  local loaderFile="$ZSH_EVALCACHE_DIR/completion/_${name##*/}"
  ...
  if [ "$ZSH_EVALCACHE_DISABLE" = "true" ]; then
    ...
    if type "${name}" > /dev/null; then
      # echo "evalcache: ${name} initialization not cached, caching output of: $*" >&2
      mkdir -p "$ZSH_EVALCACHE_DIR/completion/cache"
      eval ${(q)@} > "$cacheFile"
      # zcompile "$cacheFile"
      # source "$cacheFile"
    else
  ...
  # echo "#compdef ${name}\nsource "$cacheFile"; _${name##*/};" > "$loaderFile"
  # echo "#compdef _${name} ${name}\n_${name}() {\n  source "$cacheFile";\n  _${name##*/} "\$\@";\n}" > "$loaderFile"
  eval "compdef _${name} ${name};function _${name} () {source "$cacheFile"; _${name##*/} "\$\@";}"
  # function _${name} {
  #   source "$cacheFile"
  #   _${name} $@;
  #   }
  # function _${name##*/} {echo "$cacheFile"; source "$cacheFile"; _${name##*/} "$@";}
}
